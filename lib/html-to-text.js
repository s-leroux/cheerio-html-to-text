...
//  console.log("data is %s", data);
  ...
  let head = node.makeRootNode(options.width);
  
  function enter(element) {
    if (element.type == 'tag')
      head = node.makeNode(head);
    else if (element.type == 'text')
      head.content.push(text.makeInlineText(element.data));
  }
  
  function leave(element) {
    if (element.type == 'tag') {
      head = node.popNode(head, elements[element.name].pop);
    }
  }
